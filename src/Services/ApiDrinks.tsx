import { DrinkCategoryDetailsType, DrinkCategoryType, DrinkType } from '../utils/types';
...
export const fetchDrinksByCategory = async (): Promise<DrinkCategoryType[]> => {
  const response = await fetch(`${BASE_URL}/list.php?c=list`);
  const data = await response.json();
  return data.drinks;
};
export const fetchFilterDrinksByCategory = async (cat:string):
Promise<DrinkCategoryDetailsType[]> => {
